...
Write-Host '[INFO] Uploading files in batches to avoid timeout...'
...
foreach ($item in $items) {
    if (Test-Path $item) {
        $uploadedCount++
        Write-Host "  [$uploadedCount/$totalItems] Uploading $item..."
...
$permCmd = 'cd ~/pse-vision && chmod +x *.sh 2>/dev/null; chmod +x build_desktop.sh setup_service.sh remove_service.sh 2>/dev/null; echo Permissions set'
